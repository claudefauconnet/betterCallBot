var neo4jProxy = (function () {
        ...
        self.queryQuestionConceptsSimple = function (words, callback) {
            ...
            words.forEach(function (word, index) {
                ...
                if (index == 0) {
                    match = " match(n:" + type + ")-[r2]-(f:paragraph) where n.name=\"" + word + "\" "
            ...
            match += " with f match (f)-[r]-(p:chapter)-[r2]-(F:file)  return distinct f,p,F"
            console.log(match);
        ...
        self.queryQuestionWordsSimple = function (words, callback) {
            ...
            words.forEach(function (word, index) {
                ...
                if (index == 0) {
                    match = " match(n:noun)-[r]-(f:paragraph) where n.name=\"" + word + "\" "
                ...
                for (var key2 in file.chapters) {
                    ...
                    chapter.forEach(function (paragraph) {
                        ...
                        tokens.nouns.forEach(function (noun) {
                            ...
                            statements.push({statement: "match (n:paragraph { name: \"" + paragraph.id + "\"}), (m:noun{ name: \"" + noun.word.toLowerCase() + "\"}) create (n)-[:hasNoun" + attrs + "]->(m)"});
                        ...
                        tokens.numValues.forEach(function (numValue) {
                            ...
                            statements.push({statement: "match (n:paragraph { name: \"" + paragraph.id + "\"}), (m:numValue { name: \"" + numValue.word + "\"}) create (n)-[:hasNumValue" + attrs + "]->(m)"});
            ...
            if (coreNlp.openieTriples) {
                ...
                for (var i = 0; i < coreNlp.openieTriples.length; i++) {
                    ...
                    if (i == 0) {
                        ...
                        statements.push({statement: "MERGE (n:docParagraph { name: \"" + tripleObj.id + "\",subGraph:\"totalRef\"})"});
                        statements.push({statement: "match (n:conceptSource { name: \"" + tripleObj.concept + "\"}), (m:docParagraph{ name: \"" + tripleObj.id + "\"}) create (n)-[:indocParagraph]->(m)"});
                    }
                    ...
                    for (var j = 0; j < tripleObj.triples.length; j++) {
                        ...
                        statements.push({statement: "match (n:docParagraph { name: \"" + tripleObj.id + "\"}), (m:subject{ name: \"" + triple.subject + "\"}) create (n)-[:hasSubject]->(m)"});
            ...
            if (coreNlp.currentTokens) {
                ...
                for (var i = 0; i < coreNlp.currentTokens.length; i++) {
                    ...
                    if (true || i == 0) {
                        ...
                        statements.push({statement: "MERGE (n:docParagraph { name: \"" + tokenObj.id + "\",subGraph:\"totalRef\"})"});
                        statements.push({statement: "match (n:conceptSource { name: \"" + tokenObj.concept + "\"}), (m:docParagraph{ name: \"" + tokenObj.id + "\"}) create (n)-[:indocParagraph]->(m)"});
                    }
                    if (tokenObj.tokens.sentences) {
                        for (var j = 0; j < tokenObj.tokens.sentences.length; j++) {
                            ...
                            statements.push({statement: "match (n:docParagraph { name: \"" + tokenObj.id + "\"}), (m:sentence{ uid:\"" + sentenceId + "\"}) create (n)-[:composedOf]->(m)"});
